...
네트워크 오류 뒤에 `POST`를 재시도하면 생성 작업이 두 번 실행될 수 있다. 결제, 파일 처리, 모델 작업 제출처럼 중복 실행이 위험한 API는 클라이언트가 만든 **idempotency key**를 받고, 서버가 동일한 사용자·키 조합의 결과를 저장해 재사용하도록 설계한다. 키는 인증이나 인가를 대체하지 않으며, 보존 기간·충돌 처리·요청 본문 불일치 규칙을 API 계약(OpenAPI)에 명시한다.

```http
POST /v1/prediction-jobs HTTP/1.1
Idempotency-Key: 9a49b5d2-...
Content-Type: application/json
```
...
| 인증이 필요한 엔드포인트가 명확한가? | ☐ |
| 생성 요청의 재시도·중복 실행 규칙이 API 계약에 있는가? | ☐ |
