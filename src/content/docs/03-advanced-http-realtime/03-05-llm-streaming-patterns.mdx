...
FastAPI의 `StreamingResponse`는 async generator를 스트림으로 내보내는 기본 도구다. 단, 오래 걸리는 모델 생성이나 별도 스레드 작업을 붙일 때는 연결 해제를 "항상 자동으로 즉시 중단된다"고 가정하지 말고, `await asyncio.sleep(0)`처럼 취소 지점을 두거나 `Request.is_disconnected()`로 클라이언트 연결 상태를 확인하는 패턴을 함께 고려한다.
