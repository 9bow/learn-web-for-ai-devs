...
## MPA vs SPA: AI 개발자 관점

AI 엔지니어가 모델 서빙 서버(FastAPI)를 만들 때 프론트엔드를 어떻게 선택할지 고민이 생긴다. MPA 방식 — 즉 Django나 FastAPI의 Jinja2 템플릿으로 HTML을 직접 렌더링하는 것 — 은 여전히 유효한 선택지다.

| 선택 기준 | MPA (템플릿) | SPA (React 등) |
|----------|-------------|----------------|
| 팀 규모 | 혼자 또는 소규모 | 프론트/백 분리 팀 |
| 인터랙션 수준 | 폼 제출, 페이지 이동 | 실시간 업데이트, 복잡한 상태 |
| SEO 필요성 | 있음 (블로그, 문서) | 내부 대시보드면 불필요 |
| 배포 복잡도 | 단순 (단일 서버) | 높음 (정적 빌드 + API 서버) |

Hugging Face의 Spaces 같은 ML 데모 플랫폼은 Gradio — 본질적으로 Python 기반 MPA — 를 기본으로 쓴다. 빠른 프로토타이핑에는 MPA가 오히려 적합하다. 사용자와 인터랙션이 복잡해지는 시점에 SPA로 전환을 고려한다.

## 핵심 정리

- **MPA는 서버가 매 요청마다 완성된 HTML을 생성해 응답하는 구조다** — PHP/JSP/Django 템플릿이 핵심 기술이다
- **모든 인터랙션이 전체 페이지 리로드를 유발한다** — 헤더/네비게이션도 매번 재전송·재렌더링된다
- **2004~2005년 Gmail·Google Maps 등장이 전환점이었다** — "웹도 앱처럼 동작할 수 있다"는 기대가 생겼다
- **MPA는 지금도 유효한 선택이다** — 빠른 프로토타이핑, SEO 필요 사이트, 소규모 팀에 적합하다
- **복잡한 인터랙션이 필요해질 때 SPA로 전환한다** — AJAX와 JavaScript 생태계가 그 다리 역할을 했다
