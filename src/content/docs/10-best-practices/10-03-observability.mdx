...
learning_objectives:
  ...
  - "OpenTelemetry semantic conventions를 따라 요청 경로를 추적하고 AI telemetry의 민감 정보를 구분할 수 있다"
prerequisites:
  - "10-02-performance"
...
프로덕션 서비스는 "왜 느려졌는가?", "어떤 사용자에게 에러가 났는가?", "ML 모델 정확도가 떨어졌는가?"를 빠르게 답할 수 있어야 한다. 이를 가능하게 하는 것이 **관측성(Observability)**의 세 기둥: 로그, 메트릭, 트레이싱이다. 서로 다른 서비스가 같은 이름과 attribute를 쓰려면 OpenTelemetry semantic conventions를 기준으로 삼는다.
...
## AI telemetry: 유용하지만 원문은 기본 수집하지 않는다

AI 요청에는 모델 이름, 제공자, 지연 시간, 성공·실패, 입력·출력 token 수, 비용 추정치 같은 운영 지표가 유용하다. 하지만 prompt, completion, 문서 조각, API key, 사용자 식별자는 민감할 수 있다. 기본 로그·span attribute에는 원문을 넣지 않고, 필요한 경우에도 최소화·마스킹·접근 통제를 먼저 설계한다.

```text
요청 ID ──▶ HTTP span ──▶ retrieval span ──▶ model span
                 │                │                │
            상태·지연          문서 개수        모델·token 수
            (원문 제외)        (원문 제외)      (원문 제외)
```

OpenTelemetry의 GenAI semantic conventions는 별도 공식 저장소에서 계속 발전한다. 특정 attribute 이름을 코드에 고정하기보다, 적용 중인 SDK와 해당 버전의 규약을 함께 기록한다.
