...
```text
사용자 API 키: 브라우저에 장기 저장하지 않고 서버의 비밀 관리 저장소에 보관
...
다른 출처로 JavaScript fetch 요청을 보내면 브라우저는 응답을 읽을 수 있는지 CORS 정책으로 판단합니다. CORS는 **서버의 인증·인가를 대체하지 않으며**, curl·모바일 앱·공격자 서버 같은 비브라우저 클라이언트는 이 정책을 따르지 않습니다. 쿠키가 자동 전송되는 교차 출처 요청의 위협은 CSRF 방어, 인증·인가, `SameSite` 같은 별도 통제로 다뤄야 합니다.
...
- CORS는 브라우저의 응답 접근 제어이지 API 인증·인가나 CSRF 방어 수단이 아니다.
