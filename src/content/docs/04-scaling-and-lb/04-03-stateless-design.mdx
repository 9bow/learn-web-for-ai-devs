...
자체 포함형 JWT의 **기본 검증 경로**는 세션 저장소를 요구하지 않습니다. 서버가 늘어나도 각 인스턴스가 같은 검증 키로 서명과 만료를 확인할 수 있습니다. 다만 계정 비활성화, 강제 로그아웃, 토큰 폐기 목록을 즉시 반영하려면 DB나 Redis 조회를 추가할 수 있으므로 JWT를 쓴다고 애플리케이션 전체가 자동으로 무상태가 되는 것은 아닙니다.

| 방식 | 서버 상태 | 토큰 무효화 | 확장성 |
|------|----------|------------|--------|
| 메모리 세션 | 있음 (WAS 메모리) | 즉시 가능 | 불가 |
| Redis 세션 | 있음 (Redis) | 즉시 가능 | 가능 |
| 자체 포함형 JWT | 기본 검증에는 없음 | 별도 폐기 저장소가 없으면 만료 전까지 어려움 | 높음 |
...
- 자체 포함형 JWT는 기본 검증에 세션 저장소가 필요 없어 확장에 유리하지만, 즉시 폐기를 구현하면 별도 상태 저장소가 필요하다.
