...
## AI 서비스에서 Frontend 서버가 필요한가

AI 모델을 FastAPI로 서빙할 때 Next.js가 항상 필요한 것은 아니다.

```text
시나리오 A: 내부 대시보드 / ML 실험 UI
  → CSR(Create React App / Vite) 충분
  → SEO 불필요, 팀 내부 사용
  → 정적 빌드를 nginx로 서빙

시나리오 B: 사용자 대면 AI 서비스 (검색, 추천, 챗봇 랜딩)
  → SSR 또는 SSG 필요
  → 검색 노출 필요, 초기 로딩 속도 중요
  → Next.js + FastAPI 분리 구조

시나리오 C: 빠른 프로토타입
  → Gradio, Streamlit — Python MPA
  → Node 서버 없이 FastAPI 하나로 해결
```

이 커리큘럼의 실습(섹션 09)은 시나리오 A다. nginx + Vite 빌드 + FastAPI 조합으로, Next.js 없이 완전한 풀스택 구조를 만든다. Next.js는 SEO와 성능 요구사항이 생길 때 도입한다.
...
- **AI 서비스 특성에 따라 선택한다** — 내부 대시보드는 CSR, 공개 서비스는 SSR/SSG가 적합하다
