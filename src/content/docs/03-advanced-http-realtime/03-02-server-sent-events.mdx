...
learning_objectives:
  ...
  - "FastAPI StreamingResponse, 프록시 버퍼링, 연결 종료·재연결 처리를 함께 점검할 수 있다"
prerequisites:
  - "03-01-websocket-basics"
...
`X-Accel-Buffering: no` 헤더는 nginx에게 이 응답을 버퍼링하지 말라고 지시한다. 다만 실제 전달 여부는 프록시와 CDN 설정에도 달려 있으므로, 브라우저 개발자 도구에서 이벤트가 점진적으로 도착하는지 확인한다.
...
`EventSource`는 GET 요청만 지원한다. POST 바디로 프롬프트를 보내야 하는 경우에는 `fetch` API로 응답 body를 읽는 방식을 사용한다. 이때 네트워크 청크가 SSE 이벤트 경계와 일치하지 않으므로 청크마다 단순 줄 분할하지 말고, 남은 버퍼를 보존하는 파서를 사용해야 한다.
...
`proxy_buffering off`를 설정하지 않으면 nginx가 백엔드 응답을 버퍼에 모았다가 한꺼번에 보내서 실시간 스트리밍이 깨질 수 있다. 타임아웃은 모델의 최대 응답 시간과 heartbeat 정책에 맞추고, 재연결을 지원한다면 `id:`와 `Last-Event-ID`를 실제 저장 가능한 이벤트 순서와 연결한다.
...
- nginx에서는 `proxy_buffering off`와 `X-Accel-Buffering: no`를 검토하고, 실제 CDN·프록시 경로에서 점진 전달을 검증한다
