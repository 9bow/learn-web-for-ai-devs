...
`async`는 주로 네트워크·DB 같은 I/O 대기를 효율적으로 처리한다. CPU bound 작업은 프로세스·작업 큐 같은 별도 실행 경계를 검토하고, GPU 모델 추론은 모델 서버의 동시성·배치·메모리 정책에 맞춰 별도 worker로 운영한다. 단순히 모든 추론을 웹 프로세스의 thread pool에 넣는 것은 모델 라이브러리와 GPU 자원 경합을 숨길 수 있다.

```python
@app.post("/api/prediction-jobs", status_code=202)
async def submit_prediction(data: PredictRequest):
    # 예시: 검증 뒤 별도 worker queue에 작업을 제출한다.
    job_id = await job_queue.submit(data.model_dump())
    return {"job_id": job_id, "status": "queued"}
```

짧고 가벼운 CPU 작업에 executor가 적합할 수는 있지만, worker 수·큐 길이·취소·재시도·모델 메모리의 소유자를 명시적으로 관리해야 한다.
...
GPU 배치는 처리량을 높일 수 있지만 대기열 때문에 지연 시간이 늘 수도 있다. 배치 크기와 대기 시간은 모델·GPU 메모리·SLO로 측정해 정한다.
